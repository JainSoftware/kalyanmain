...
require_once(__DIR__ . '/../translations/admin_translations/trans-users-trash.php');
...
if($total_rows > 0) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		// profile pic path
		$this_pic_path = __DIR__ . '/../profile_pic_full/' . $folder . '/' . $this_id;
		...
		if(!empty($this_pic_glob_arr)) {
			...
			$this_prof_pic_url = $baseurl . '/profile_pic_full/' . $folder . '/' . $this_prof_pic_filename;
		}
		else {
			$this_prof_pic_url = '';
		}
